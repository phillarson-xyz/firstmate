...
That path evidence is confined to the harness names themselves: whether a process is instead a bare interpreter running a harness script is decided from the executable basename alone, because tool bridges ship as native binaries below `node_modules` and a path that merely says `node` would otherwise let one of them claim the session.
`fm_is_bare_interpreter` in `bin/fm-cursor-lib.sh` owns that rule for both the ancestry walk and Cursor identity, which ask it of the same live process, while `bin/fm-harness.sh` and `bin/fm-gemini-lib.sh` keep their own separate lists.
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees, and drives a tool bridge under `node_modules` apart from real `node`, `nodejs`, versioned-Node, and `python` interpreters carrying the same harness script argument.
