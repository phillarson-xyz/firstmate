...
- Helper scripts in `bin/` are primarily bash, alongside the Python helpers already tracked there.
  ...
  A Nix host needs neither installer: `flake.nix`'s dev shell carries both pinned builds, and `bin/fm-lint.sh` re-runs itself in that shell when ShellCheck is not on `PATH`.
